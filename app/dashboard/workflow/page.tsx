...
export default function WorkflowPage() {
  ...
  async function saveActivity(event: FormEvent) {
    ...
    await run(
      () =>
        api(
          id ? `/api/activities/${id}` : "/api/activities",
          {
            method: id ? "PATCH" : "POST",
            body: JSON.stringify(activity),
          },
        ),
      id ? "Activity updated." : "Activity created.",
    );
  ...
  function editActivity(item: Row) {
    setEditingActivityId(String(item.id));
    setActivity({
  ...
  function editReport(item: Row) {
    setEditingReportId(String(item.id));
    setReport({
      projectId: String(item.projectId || ""),
  ...
  async function saveIndicator(event: FormEvent) {
    event.preventDefault();

    const id = editingIndicatorId;

    await run(
      () =>
        api(
          id ? `/api/indicators/${id}` : "/api/indicators",
          {
            method: id ? "PATCH" : "POST",
            body: JSON.stringify({
              ...indicator,
      ...
      id ? "Indicator updated." : "Indicator created.",
    );
  ...
  function editIndicator(item: Row) {
    setEditingIndicatorId(String(item.id));
    setIndicator({
      projectId: String(item.projectId || ""),
      ...
      description: String(item.description || ""),
      level: String(item.level || "OUTPUT"),
  ...
  async function saveTarget(event: FormEvent) {
    ...
    await run(
      ...
      id ? "Target updated." : "Target created.",
    );
  ...
  function editTarget(item: Row) {
    setEditingTargetId(String(item.id));
    setTarget({
  ...
  async function saveResult(event: FormEvent) {
    ...
    const id = editingResultId;

    await run(() => api(
      id ? `/api/results/${id}` : "/api/results",
      {
        method: id ? "PATCH" : "POST",
        body: JSON.stringify({
          ...result,
    ...
    ), id ? "Result updated." : "Result recorded.");

    setResult(initialResult);
    setEditingResultId("");
  }

  function editResult(item: Row) {
    setEditingResultId(String(item.id));
    setResult({
      ...
      <section className="workspace">
        ...
        <div className="page-content workflow-page">
          ...
          <div className="workflow-grid">
            ...
            <section className="flow-card" id="activities"><div className="flow-head"><div><p className="eyebrow dark">04 · ACTIVITY</p><h2>{editingActivityId ? "Edit activity" : "Create activity"}</h2></div><div className="flow-number">04</div></div><form onSubmit={saveActivity}><label>Project<select value={activity.projectId} onChange={(e) => setActivity({ ...activity, projectId: e.target.value })} required><option value="">Select project</option>{projects.map((item) => <option key={String(item.id)} value={String(item.id)}>{String(item.code)} · {String(item.name)}</option>)}</select></label><label>Title<input value={activity.title} onChange={(e) => setActivity({ ...activity, title: e.target.value })} required /></label><label>Description<textarea value={activity.description} onChange={(e) => setActivity({ ...activity, description: e.target.value })} /></label><label>Due date<input type="date" value={activity.dueDate} onChange={(e) => setActivity({ ...activity, dueDate: e.target.value })} /></label><div style={{display:"flex",gap:"7px"}}><button className="form-button" disabled={busy}>{editingActivityId ? "Save changes" : "Create activity"}</button>{editingActivityId && <button type="button" className="form-button secondary-button" onClick={cancelActivityEdit} disabled={busy}>Cancel</button>}</div></form><div className="record-list">{activities.slice(0,4).map((item) => <div key={String(item.id)}><strong>{String(item.approvalStatus)}</strong><span>{String(item.title)}</span><div style={{display:"flex",gap:"8px"}}><button onClick={() => editActivity(item)} disabled={busy}>Edit</button><button onClick={() => run(() => api(`/api/activities/${item.id}`, { method: "DELETE" }), "Activity deleted.")} disabled={busy}>Delete</button></div></div>)}</div></section>
            <section className="flow-card approval" id="approval">
              <div className="flow-head">
                <div>
                  <p className="eyebrow dark">05 · APPROVAL</p>
              ...
              {(() => {
                const submittedActivities = activities.filter(
                  (item) => String(item.approvalStatus) === "SUBMITTED"
                );

                const selectedSubmitted =
                  submittedActivities.find(
                    (item) => String(item.id) === selectedActivity
                  ) || submittedActivities[0];

                const selectedId = selectedSubmitted
                  ? String(selectedSubmitted.id)
                  : "";
                    ...
                    {submittedActivities.length === 0 ? (
                      <div className="record-list">
                        <div>
                          ...
                          <span>No submitted activities require a decision.</span>
                        </div>
                      </div>
                    ) : (
                      <>
                        <label>
                          Activity awaiting decision
                          <select
                            value={selectedId}
                            onChange={(e) => setSelectedActivity(e.target.value)}
                          >
                            {submittedActivities.map((item) => (
                              <option
                                key={String(item.id)}
                        ...
                        <div className="approval-status">
                          SUBMITTED
                          {selectedSubmitted?.submittedAt
                            ? ` · submitted ${new Date(
                                String(selectedSubmitted.submittedAt)
                              ).toLocaleString()}`
                            : ""}
                        </div>
                          ...
                          <button
                            ...
                            onClick={() =>
                              run(
                                () =>
                                  api(
                                    `/api/activities/${selectedId}/approve`,
                                    { method: "POST" }
                                  ),
                                "Activity approved."
                              )
                          ...
                          <button
                            ...
                            onClick={() =>
                              run(
                                () =>
                                  api(
                                    `/api/activities/${selectedId}/reject`,
                                    {
                                      method: "POST",
                                      body: JSON.stringify({
            ...
            <section className="flow-card report" id="reports">
              <div className="flow-head">
                <div>
                  <p className="eyebrow dark">06 · REPORT</p>
                  <h2>{editingReportId ? "Edit report" : "Create / read report"}</h2>
                </div>
              ...
              <form onSubmit={saveReport}>
                <label>
                  ...
                  <select
                    ...
                    onChange={(e) => setReport({ ...report, projectId: e.target.value })}
                    required
                  >
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) => setReport({ ...report, periodStart: e.target.value })}
                    required
                  />
                </label>
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(e) => setReport({ ...report, periodEnd: e.target.value })}
                    required
                  />
                </label>
                ...
                <label>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setReport({ ...report, narrative: e.target.value })}
                    required
                  />
                </label>
              ...
              <div className="record-list">
                {reports.slice(0, 4).map((item) => (
                  <div key={String(item.id)}>
                    <strong>{String(item.periodStart)}</strong>
                    <span>{String(item.narrative).slice(0, 70)}</span>
            ...
            <section className="flow-card" id="indicators">
              <div className="flow-head"><div><p className="eyebrow dark">07 · INDICATOR</p><h2>{editingIndicatorId ? "Edit indicator" : "Create indicator"}</h2></div><div className="flow-number">07</div></div>
              <form onSubmit={saveIndicator}>
                ...
                <div style={{display:"flex",gap:"7px"}}>
                  <button className="form-button" disabled={busy}>
                    {editingIndicatorId ? "Save changes" : "Create indicator"}
                  </button>
                  {editingIndicatorId && (
                    <button
                      type="button"
                      className="form-button secondary-button"
                      onClick={cancelIndicatorEdit}
                      disabled={busy}
                    >
                      Cancel
                    </button>
              ...
              <div className="record-list">{indicators.slice(0,4).map((item) => <div key={String(item.id)}><strong>{String(item.code)}</strong><span>{String(item.name)} · {String(item.unit)}</span><div style={{display:"flex",gap:"8px"}}><button onClick={() => editIndicator(item)} disabled={busy}>Edit</button><button onClick={() => run(() => api(`/api/indicators/${item.id}`, { method: "DELETE" }), "Indicator deleted.")} disabled={busy}>Delete</button></div></div>)}</div>
            </section>

            <section className="flow-card" id="targets">
              <div className="flow-head"><div><p className="eyebrow dark">08 · TARGET</p><h2>{editingTargetId ? "Edit target" : "Set target"}</h2></div><div className="flow-number">08</div></div>
              <form onSubmit={saveTarget}>
                ...
                <div style={{display:"flex",gap:"7px"}}>
                  <button className="form-button" disabled={busy || !target.indicatorId}>
                    {editingTargetId ? "Save changes" : "Set target"}
                  </button>
                  {editingTargetId && (
                    <button
                      type="button"
              ...
              <div className="record-list">{targets.slice(0,4).map((item) => <div key={String(item.id)}><strong>{String(item.year)}</strong><span>{String(item.indicatorCode)} · {String(item.targetValue)}</span><div style={{display:"flex",gap:"8px"}}><button onClick={() => editTarget(item)} disabled={busy}>Edit</button><button onClick={() => run(() => api(`/api/targets/${item.id}`, { method: "DELETE" }), "Target deleted.")} disabled={busy}>Delete</button></div></div>)}</div>
            </section>

            <section className="flow-card report" id="results">
              <div className="flow-head"><div><p className="eyebrow dark">09 · RESULT</p><h2>{editingResultId ? "Edit result" : "Record actual performance"}</h2></div><div className="flow-number">09</div></div>
              <form onSubmit={saveResult}>
                ...
                <div style={{display:"flex",gap:"7px"}}>
                  <button className="form-button" disabled={busy || !result.targetId}>
                    {editingResultId ? "Save changes" : "Record result"}
                  </button>
                  {editingResultId && (
                    <button
                      type="button"
                      className="form-button secondary-button"
                      onClick={cancelResultEdit}
                      disabled={busy}
                    >
                      Cancel
                    </button>
              ...
              <div className="record-list">{results.slice(0,4).map((item) => <div key={String(item.id)}><strong>{String(item.actualValue)}</strong><span>{String(item.indicatorCode)} · {String(item.periodStart)} → {String(item.periodEnd)}</span><div style={{display:"flex",gap:"8px"}}><button onClick={() => editResult(item)} disabled={busy}>Edit</button><button onClick={() => run(() => api(`/api/results/${item.id}`, { method: "DELETE" }), "Result deleted.")} disabled={busy}>Delete</button></div></div>)}</div>
            </section>
          </div>
        </div>
      </section>
