...
import {
  deliverySiteBelongsToDistrict,
  resolveDeliverySiteId,
  resolveDistrictId,
} from "@/lib/geography";
...
export async function GET(request: NextRequest) {
  ...
  try {
    return NextResponse.json(
      await database().select().from(interventions).orderBy(desc(interventions.interventionDate)),
    );
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const deliverySiteId = parsed.data.deliverySiteId
      ? await resolveDeliverySiteId(parsed.data.deliverySiteId)
      : null;
    if (parsed.data.deliverySiteId && !deliverySiteId) {
      return NextResponse.json({ error: "Delivery site not found." }, { status: 404 });
    }
    if (deliverySiteId && !(await deliverySiteBelongsToDistrict(deliverySiteId, districtId))) {
      return NextResponse.json(
        { error: "Delivery site does not belong to the selected district." },
        { status: 422 },
      );
    }
    ...
    const [created] = await database()
      ...
      .values({ ...parsed.data, districtId, deliverySiteId })
      .returning();
