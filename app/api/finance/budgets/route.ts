...
const budgetInput = z.object({
  ...
  directorateId: z.string().uuid().optional(),
...
export async function POST(request: NextRequest) {
  ...
  const scopeId = data[scopeKey];
  if (!scopeId) return NextResponse.json({ error: `${scopeKey} is required for ${data.level} budgets.` }, { status: 422 });
  ...
  try {
    ...
    if (data.parentBudgetId) {
      const [parent] = await db.select({ id: financeBudgets.id, level: financeBudgets.level, financialYear: financeBudgets.financialYear, amountZmw: financeBudgets.amountZmw }).from(financeBudgets).where(eq(financeBudgets.id, data.parentBudgetId)).limit(1);
      ...
      parentAmount = Number(parent.amountZmw);
    ...
    const values = { ...data, amountZmw: String(data.amountZmw), status: "DRAFT" as const };
    const [created] = await db.insert(financeBudgets).values(values).returning();
    await recordAuditEvent({ actorUserId: session.userId, action: "FINANCE_BUDGET_CREATED", entityType: "finance_budget", entityId: created.id, afterValue: { level: created.level, financialYear: created.financialYear, budgetCode: created.budgetCode, amountZmw: created.amountZmw, parentBudgetId: created.parentBudgetId } });
