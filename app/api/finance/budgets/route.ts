...
const budgetInput = z.object({
  ...
  directorateId: z.string().uuid().optional(), programmeId: z.string().uuid().optional(), projectId: z.string().uuid().optional(), activityId: z.string().uuid().optional(), parentBudgetId: z.string().uuid().optional(),
...
function budgetDbError(error: unknown) {
  ...
  if (code === "22P02") return NextResponse.json({ error: "One of the supplied IDs is invalid. Please refresh the Directorate list and try again." }, { status: 422 });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (data.level === "DIRECTORATE") {
      const [entity] = await db.select({ id: directorates.id }).from(directorates).where(eq(directorates.id, data.directorateId!)).limit(1);
      if (!entity) return NextResponse.json({ error: "Directorate not found." }, { status: 404 });
      const [existingYear] = await db.select({ id: financeBudgets.id }).from(financeBudgets).where(and(eq(financeBudgets.directorateId, data.directorateId!), eq(financeBudgets.financialYear, data.financialYear), eq(financeBudgets.level, "DIRECTORATE"))).limit(1);
    ...
    if (data.parentBudgetId) {
      ...
      const parentAmount = Number(parent.amountZmw); const allocated = await db.execute(sql`select coalesce(sum(amount_zmw), 0) as total from finance_budgets where parent_budget_id = ${data.parentBudgetId} and status <> 'CLOSED'`); const alreadyAllocated = Number((allocated.rows[0] as { total?: number | string } | undefined)?.total ?? 0);
