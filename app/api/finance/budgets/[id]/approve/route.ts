...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (budget.parentBudgetId) {
      const [parent] = await db.select({ status: financeBudgets.status }).from(financeBudgets).where(eq(financeBudgets.id, budget.parentBudgetId)).limit(1);
      if (!parent || parent.status !== "APPROVED") return NextResponse.json({ error: "The parent budget must be approved before a child budget can be approved." }, { status: 422 });
    }
    ...
    await recordAuditEvent({ actorUserId: session.userId, action: "FINANCE_BUDGET_APPROVED", entityType: "finance_budget", entityId: updated.id, beforeValue: { status: budget.status }, afterValue: { status: updated.status, level: updated.level, amountZmw: updated.amountZmw } });
