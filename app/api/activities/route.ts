...
export async function GET(request: NextRequest) { const denied=requireServiceAccess(request,"activities.read"); if(denied)return denied; try{return NextResponse.json(await database().select().from(activities).orderBy(desc(activities.createdAt)));}catch(error){return apiError(error);} }
